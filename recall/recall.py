...
def yank_to_clipboard(text: str) -> str:
    """Copy to system clipboard with timeout to prevent terminal freeze."""
    for cmd in [["xclip", "-selection", "clipboard"], ["xsel", "--clipboard", "--input"], ["wl-copy"]]:
        try:
            ...
            continue
    return "install xclip / xsel / wl-copy (or check $DISPLAY / Wayland)"


def yank_to_primary(text: str) -> str:
    """Copy to primary selection with timeout."""
    for cmd in [["xclip", "-selection", "primary"], ["xsel", "--primary", "--input"], ["wl-copy", "--primary"]]:
        try:
            result = subprocess.run(
                cmd,
                input=text.encode(),
...
VALID_REGISTERS = set('"0123456789abcdefghijklmnopqrstuvwxyz+*')


def build_preview_lines(entry: dict) -> list[str]:
    match = entry.get("_match") or {}
    hits = entry.get("_hits") or ([] if not match else [match])
    tags_text = ", ".join(entry.get("tags", [])) or "(none)"
    lines = [
        f"Title    {entry.get('title', '')}",
        f"Category {CAT_LABEL.get(entry.get('category', ''), entry.get('category', '').upper())}",
        f"Tags     {tags_text}",
        f"File     {_display_path(entry)}",
        f"Updated  {entry.get('updated_at', '')}",
    ]
    if match.get("line_text"):
        label = f"Match    line {match.get('line_no', '-')}"
        if match.get("column"):
            label += f" col {match.get('column')}"
        if len(hits) > 1:
            label += f"   +{len(hits) - 1} more"
        lines.append(label)
        lines.append(match.get("line_text", ""))
    lines.append("─" * 72)
    content_lines = entry.get("content", "").splitlines() or [""]
    return lines + content_lines


def _clamp_col(lines: list[str], line_idx: int, col: int) -> int:
    if not lines:
        return 0
    return max(0, min(len(lines[line_idx]), col))


def move_word_forward(line: str, col: int) -> int:
    if not line:
        return 0
...
def move_word_backward(line: str, col: int) -> int:
    if not line:
        return 0
    matches = list(re.finditer(r"\w+", line))
...
def move_word_end_forward(line: str, col: int) -> int:
    if not line:
        return 0
    matches = list(re.finditer(r"\w+", line))
    for m in matches:
...
def move_word_end_backward(line: str, col: int) -> int:
    if not line:
        return 0
    matches = list(re.finditer(r"\w+", line))
    for m in reversed(matches):
...
def selection_bounds(
    ...
    if not visual_mode:
        return None
    start = (anchor_line, anchor_col)
    end = (cursor_line, cursor_col)
    return (start, end) if start <= end else (end, start)


def line_selection_range(
    visual_mode: Optional[str],
    anchor_line: int,
    anchor_col: int,
    cursor_line: int,
    ...
    bounds = selection_bounds(visual_mode, anchor_line, anchor_col, cursor_line, cursor_col)
    if not bounds:
        return None
    ...
    if start_line == end_line:
        lo = min(start_col, end_col)
        hi = min(len(line), max(start_col, end_col) + 1)
        return (lo, hi)
    if line_idx == start_line:
        return (min(len(line), start_col), len(line))
    if line_idx == end_line:
        return (0, min(len(line), end_col + 1))
    return (0, len(line))


def selected_preview_text(
    lines: list[str],
    visual_mode: Optional[str],
    anchor_line: int,
    anchor_col: int,
    ...
    for idx, line in enumerate(lines):
        sel = line_selection_range(visual_mode, anchor_line, anchor_col, cursor_line, cursor_col, idx, line)
        if sel is None:
            continue
        start, end = sel
...
def render_selectable_preview(
    ...
    rows: list[Text] = []
    for idx, line in enumerate(lines):
        prefix = Text(f"{idx + 1:>4} ", style=PX["muted"])
        body = Text(line, style=PX["fg"])
        sel = line_selection_range(visual_mode, anchor_line, anchor_col, cursor_line, cursor_col, idx, line)
        if sel is not None:
            start, end = sel
            if end > start:
                body.stylize(f"black on {PX['yellow']}", start, end)
        if idx == cursor_line:
            prefix.stylize(f"bold black on {PX['blue']}")
            if line:
                caret_start = min(cursor_col, max(0, len(line) - 1))
                caret_end = min(len(line), caret_start + 1)
                if caret_end > caret_start:
                    body.stylize(f"bold black on {PX['cyan']}", caret_start, caret_end)
            else:
                body.append(" ", style=f"bold black on {PX['cyan']}")
        rows.append(prefix + body)
    return Group(*rows)


def preview_selection_line_span(
    visual_mode: Optional[str],
    anchor_line: int,
    cursor_line: int,
    current_line: int,
    ...
    if not visual_mode:
        return current_line == cursor_line
    lo = min(anchor_line, cursor_line)
    hi = max(anchor_line, cursor_line)
    return lo <= current_line <= hi


def preview_search_hits(lines: list[str], query: str) -> list[tuple[int, int, int]]:
    q = (query or "").strip()
    if not q:
        return []
    hits: list[tuple[int, int, int]] = []
    for line_idx, line in enumerate(lines):
        for m in re.finditer(re.escape(q), line, flags=re.IGNORECASE):
            hits.append((line_idx, m.start(), m.end()))
    return hits
...
[bold {PX['yellow']}]  recall — keys[/bold {PX['yellow']}]
  ...
  [{PX['cyan']}]y[/{PX['cyan']}]                 yank content
  [{PX['cyan']}]" + reg + y[/{PX['cyan']}]       yank to register, e.g. `"+y`, `"*Y`
  [{PX['cyan']}]:registers[/{PX['cyan']}]        show saved registers
  ...
  [bold {PX['fg']}]VIEW[/bold {PX['fg']}]
  j/k scroll  h/l move  0/$ line start/end  w/b word move  e/ge word end
  Ctrl+f/Ctrl+b page  gg/G top/bottom  / find  n/N next-prev
  v char-select  V line-select  o swap anchor/caret
  y yank selection  Y yank with path/line

  [bold {PX['fg']}]FORMS[/bold {PX['fg']}]
  j/k move  i/Enter edit  ←/→ change category  Ctrl+r paste register
  ZZ save  :q cancel
...
class VimPreview(Static):
    ...
    def __init__(self, lines: list[str], app: "RecallApp", entry: dict):
        ...
        self.lines = lines or [""]
        self._app = app
        self.entry = entry  # needed for Y (meta yank)
        ...
        self.anchor_line = 0
        self.anchor_col = 0
        self.count = ""          # numeric prefix e.g. "5"
        self.pending = ""        # for gg, ge, etc.
    ...
    def handle_key(self, k: str, c: str) -> bool:
        ...
        if k == "escape":
            if self.visual_mode:
                ...
                self.refresh()
                return True
            return False  # let RecallApp handle global escape
        ...
        if moved:
            ...
            self.refresh()   # immediate visual update
            return True

        return False   # not handled → let RecallApp try other keys

    # ── Internal helpers (clean & self-contained) ─────────────
    ...
    def _yank_selection(self, with_meta: bool = False):
        ...
        # Always send to system clipboard (+), non-blocking
...
class RegisterModal(ModalScreen):
    ...
    #rb {{ background: {PX['bg1']}; border: heavy {PX['purple']}; padding: 1 2; width: 84; height: auto; max-height: 92%; }}
    """
    ...
    def compose(self) -> ComposeResult:
        lines = [f"[bold {PX['purple']}] Registers [/bold {PX['purple']}]", ""]
        for name in sorted(self._registers):
            text = (self._registers.get(name) or "").replace("\n", "\\n")
            if len(text) > 72:
                text = text[:72] + "..."
            lines.append(f' [{PX["yellow"]}]"{name}[/{PX["yellow"]}] {esc(text)}')
        with Container(id="rb"):
...
class ViewModal(ModalScreen):
    ...
    def __init__(self, entry: dict):
        ...
        self._cursor_line = 0
        self._cursor_col = 0
        self._visual_mode: Optional[str] = None
        self._anchor_line = 0
        self._anchor_col = 0
        self._body_lines = build_preview_lines(entry)
        self._find_mode = False
        self._find_query = ""
        self._find_hits: list[tuple[int, int, int]] = []
        self._find_index = -1
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="vo"):
            ...
            with ScrollableContainer(id="vscroll"):
                yield Static("", id="vcontent", markup=False)
            yield Static(" j/k h/l 0/$ w/b e/ge Ctrl+f/b gg/G  / find n/N next  v/V select o swap  y/Y yank  q back", id="vhint")
    ...
    def on_key(self, event: events.Key) -> None:
        ...
        if hasattr(self.app, "_register_pending") and self.app._register_pending:
            target = c or k
            self.app._register_pending = False
            if target in VALID_REGISTERS:
                self.app._active_register = target
                self.app._draw_bars()
            else:
                self.dismiss(f"status:invalid register: {target}")
            return
        if self._find_mode:
            if k == "escape":
                self._find_mode = False
        ...
        if self._pending == "g" and k == "g":
            ...
            return
        if c == '"':
            if hasattr(self.app, "_register_pending"):
                self.app._register_pending = True
                self.app._draw_bars()
            return
        if k == "y":
            self._pending = ""
            self.dismiss(f"status:{self._store_yank(self._selected_text())}")
            return
        if k == "Y":
            self._pending = ""
            self.dismiss(f"status:{self._store_yank(self._selected_text_with_meta())}")
            return
        if k == "g":
            self._pending = k
            return
        if self._pending == "g" and k == "e":
            self._pending = ""
            line = self._body_lines[self._cursor_line]
            self._cursor_col = move_word_end_backward(line, self._cursor_col)
            self._refresh_view()
            return
        self._pending = ""

        if k == "escape" and self._visual_mode:
            ...
            self._cursor_col = max(0, len(self._body_lines[self._cursor_line]) - 1)
            self._refresh_view()
        elif k == "w":
            line = self._body_lines[self._cursor_line]
            self._cursor_col = move_word_forward(line, self._cursor_col)
            self._refresh_view()
        elif k == "b":
            line = self._body_lines[self._cursor_line]
            self._cursor_col = move_word_backward(line, self._cursor_col)
            self._refresh_view()
        elif k == "e":
            line = self._body_lines[self._cursor_line]
            self._cursor_col = move_word_end_forward(line, self._cursor_col)
            self._refresh_view()
        elif k == "ctrl+f" or k == "pagedown":
            self._move_line(16)
        elif k == "ctrl+b" or k == "pageup":
            self._move_line(-16)
        elif k == "ctrl+d":
            self._move_line(8)
        elif k == "ctrl+u":
            ...
            self._cursor_line = max(0, len(self._body_lines) - 1)
            self._sync_cursor_col()
            self._refresh_view()
        elif k == "v":
            self._toggle_visual("char")
            ...
            self._toggle_visual("line")
        elif k == "o" and self._visual_mode:
            self._anchor_line, self._cursor_line = self._cursor_line, self._anchor_line
            self._anchor_col, self._cursor_col = self._cursor_col, self._anchor_col
            self._refresh_view()
        elif c == "/":
    ...
    def _toggle_visual(self, mode: str) -> None:
        if self._visual_mode == mode:
            self._visual_mode = None
        else:
    ...
    def _move_line(self, delta: int) -> None:
        if not self._body_lines:
            return
        self._cursor_line = max(0, min(len(self._body_lines) - 1, self._cursor_line + delta))
        self._sync_cursor_col()
        self._refresh_view()

    def _move_col(self, delta: int) -> None:
        line = self._body_lines[self._cursor_line] if self._body_lines else ""
        self._cursor_col = max(0, min(max(0, len(line) - 1), self._cursor_col + delta))
        self._refresh_view()

    def _sync_cursor_col(self) -> None:
        self._cursor_col = _clamp_col(self._body_lines, self._cursor_line, self._cursor_col)

    def _selected_text(self) -> str:
        return selected_preview_text(
            self._body_lines,
            self._visual_mode,
            self._anchor_line,
            self._anchor_col,
    ...
    def _selected_text_with_meta(self) -> str:
        line_no = self._cursor_line + 1
        if self._visual_mode:
            line_no = min(self._anchor_line, self._cursor_line) + 1
        meta = f"{_display_path(self._entry)}:{line_no}:{self._cursor_col + 1}"
        text = self._selected_text()
        return f"{meta}\n{text}" if text else meta

    def _store_yank(self, text: str) -> str:
        if hasattr(self.app, "_store_yank"):
            return self.app._store_yank(text)
        return yank_to_clipboard(text)
    ...
    def _refresh_view(self) -> None:
        bar = f"[{CAT_COLOR[self._entry['category']]}]{CAT_LABEL[self._entry['category']]}[/{CAT_COLOR[self._entry['category']]}]  [bold {PX['fg']}]{esc(self._entry['title'])}[/bold {PX['fg']}]"
        if self._find_mode or self._find_query:
            suffix = f"  [{PX['yellow']}]/[/{PX['yellow']}][{PX['fg']}]{esc(self._find_query)}[/{PX['fg']}]"
            if self._find_hits:
                suffix += f" [{PX['muted']}]({self._find_index + 1}/{len(self._find_hits)})[/{PX['muted']}]"
            bar += suffix
        self.query_one("#vbar", Static).update(bar)
        self.query_one("#vcontent", Static).update(self._render_body())
        self._sync_scroll()

    def _sync_scroll(self) -> None:
        sc = self.query_one("#vscroll", ScrollableContainer)
        top = max(0, self._cursor_line - 6)
        sc.scroll_to(y=top, animate=False)

    def _refresh_find_hits(self) -> None:
        self._find_hits = preview_search_hits(self._body_lines, self._find_query)
        self._find_index = 0 if self._find_hits else -1

    def _jump_find(self, step: int, restart: bool = False) -> None:
        if restart:
            self._refresh_find_hits()
        if not self._find_hits:
            ...
            return
        if not restart:
            self._find_index = (self._find_index + step) % len(self._find_hits)
        line_idx, start, _ = self._find_hits[self._find_index]
...
class FormModal(ModalScreen):
    ...
    def __init__(self, entry: Optional[dict] = None):
        ...
        self._cmdbuf = ""
        self._zbuf = ""
        self._paste_register_pending = False
    ...
    def on_key(self, event: events.Key) -> None:
        ...
        if self._editing:
            if k == "escape":
                ...
                event.stop()
                event.prevent_default()
                self._paste_active_register()
            return

        event.stop()
        event.prevent_default()
        if self._paste_register_pending:
            self._paste_register_pending = False
            target = c or k
            if hasattr(self.app, "_get_register_text"):
                text = self.app._get_register_text(target)
                self._paste_text_into_selected(text)
                self._redraw_status(f'pasted "{target}"')
            return
        if self._cmdbuf:
            if k == "escape":
                self._cmdbuf = ""
        ...
        if k in ("j", "down", "tab"):
            ...
            self._paste_register_pending = True
            self._redraw_status('Ctrl+R "')
        elif c == ":":
            self._cmdbuf = ":"
            self._redraw_status(":")
        elif k in ("q", "escape"):
    ...
    def _redraw_status(self, cmd_hint: str = ""):
        ...
        elif self._paste_register_pending:
            st.update(f"[bold {PX['yellow']}]Ctrl+R \"[/bold {PX['yellow']}][{PX['muted']}] choose register[/{PX['muted']}]")
        elif self._editing:
            st.update(f"[bold {PX['blue']}]-- INSERT --[/bold {PX['blue']}]  [{PX['muted']}]Esc exit   Ctrl+O external editor[/{PX['muted']}]")
        else:
            st.update(f"[{PX['muted']}]-- NAV --  j/k move   i/Enter edit   Ctrl+R paste   ← → cat   ZZ/:w save   :q/q cancel[/{PX['muted']}]")
    ...
    def _paste_active_register(self):
        text = self.app._get_register_text() if hasattr(self.app, "_get_register_text") else ""
        self._paste_text_into_selected(text)
        self._redraw_status("pasted register")

    def _paste_text_into_selected(self, text: str):
        if self._nav_idx == 1 or not text:
            return
        if self._nav_idx == 0:
            widget = self.query_one("#f-title", Input)
            widget.value = widget.value + text
        elif self._nav_idx == 2:
            widget = self.query_one("#f-tags", Input)
            widget.value = widget.value + text
        elif self._nav_idx == 3:
            widget = self.query_one("#f-source_path", Input)
            widget.value = widget.value + text
        elif self._nav_idx == 4:
            widget = self.query_one("#f-content", TextArea)
            widget.load_text(widget.text + text)
...
class RecallApp(App):
    ...
    def __init__(self):
        ...
        self._preview_visual_mode: Optional[str] = None
        self._preview_anchor_line = 0
        self._preview_anchor_col = 0
        self._preview_lines: list[str] = []
        self._preview_entry_id: Optional[int] = None
        self._preview_find_mode = False
        self._preview_find_query = ""
        self._preview_find_hits: list[tuple[int, int, int]] = []
        self._preview_find_index = -1
        self._registers: dict[str, str] = self._load_registers()
        self._registers.setdefault('"', "")
    ...
    def _normal(self, k: str, c: str):
        if self._register_pending:
            ...
            self._register_pending = False
            if target in VALID_REGISTERS:
                self._active_register = target
                self._draw_bars()
            else:
                self._flash(f"invalid register: {target}")
            return
        # Route all preview navigation (visual mode, find mode, motions) through VimPreview
        ...
        if k in ("j", "down"):
            ...
            self._activate_preview_visual("char")
        elif k == "V":
            self._activate_preview_visual("line")
        elif c == "a":
    ...
    def _run_command(self, cmd: str):
        if cmd in ("q", "q!", "qa", "qa!"):
            ...
            self._do_delete()
        elif cmd in ("registers", "reg"):
            self.push_screen(RegisterModal(self._registers))
    ...
    def _do_yank(self):
        entry = self._cur()
        if entry:
            self._flash(self._store_yank(entry["content"]))
    ...
    def _power_seg(self, text: str, fg: str, bg: str, next_bg: Optional[str] = None) -> str:
        seg = f"[{fg} on {bg}] {esc(text)} [/{fg} on {bg}]"
        if next_bg:
            seg += f"[{bg} on {next_bg}][/{bg} on {next_bg}]"
        return seg

    def _render_powerline(self, detail: str) -> str:
        entry = self._cur()
        mode_bg = {
            Mode.NORMAL: PX["blue"],
            Mode.SEARCH: PX["green"],
            Mode.COMMAND: PX["yellow"],
        }[self._mode]
        filter_text = (self._cat_filter or "all").upper()
        reg_text = f'reg "{self._active_register}'
        entry_text = entry["title"] if entry else "no entry"
        if len(entry_text) > 28:
            entry_text = entry_text[:28] + "..."
        visible = len(self._filtered)
        current = self._selected + 1 if visible else 0
        segs = [
            self._power_seg(self._mode.value, PX["bg"], mode_bg, PX["bg3"]),
            self._power_seg(filter_text, PX["fg"], PX["bg3"], PX["bg4"]),
            self._power_seg(reg_text, PX["fg"], PX["bg4"], PX["purple"]),
            self._power_seg(f"{current}/{visible}", PX["bg"], PX["purple"], PX["bg2"]),
            self._power_seg(entry_text, PX["fg"], PX["bg2"], PX["bg1"]),
            self._power_seg(detail, PX["muted"], PX["bg1"]),
        ]
        return "".join(segs)
    ...
    def _render_menu_bar(self) -> str:
        ...
        for i, name in enumerate(self._menu_items):
            ...
            fg = PX["bg"] if i == self._menu_idx else PX["fg"]
            label = f"[{name}]" if i == self._menu_idx else name
            next_bg = palette[(i + 1) % len(palette)] if i + 1 < len(self._menu_items) else PX["bg1"]
            segs.append(self._power_seg(label, fg, bg, next_bg))
        segs.append(self._power_seg("m cycle", PX["muted"], PX["bg1"]))
        return "".join(segs)
    ...
    def _render_title_bar(self) -> str:
        ...
        return "".join(
            [
                self._power_seg("RECALL", PX["bg"], PX["cyan"], PX["bg3"]),
                self._power_seg(f"FILTER {filter_text}", PX["fg"], PX["bg3"], PX["bg4"]),
                self._power_seg(f"ENTRY {current}/{visible}", PX["fg"], PX["bg4"], PX["purple"]),
                self._power_seg(f"DB {total}", PX["bg"], PX["purple"], PX["bg2"]),
                self._power_seg(db_path, PX["muted"], PX["bg2"]),
            ]
        )

    def _render_search_bar(self) -> str:
        active = self._mode == Mode.SEARCH
        query = self._search_q or "type to filter"
        cursor = "█" if active else ""
        hits = f"{len(self._filtered)} hits"
        mode = "LIVE" if active else "IDLE"
        return "".join(
            [
                self._power_seg("SEARCH", PX["bg"], PX["green"], PX["bg3"]),
                self._power_seg(mode, PX["fg"], PX["bg3"], PX["bg4"]),
                self._power_seg(f"/{query}{cursor}", PX["fg"], PX["bg4"], PX["bg2"]),
                self._power_seg(hits, PX["muted"], PX["bg2"], PX["bg1"]),
                self._power_seg("exact + fuzzy + rg", PX["muted"], PX["bg1"]),
            ]
        )
    ...
    def _sync_scroll(self):
        ...
        if self._preview_lines:
            psc.scroll_to(y=max(0, self._preview_cursor_line - 6), animate=False)
            return
        match = entry.get("_match") or {}
    ...
    def _handle_preview_keys(self, k: str) -> bool:
        ...
        if not entry:
            ...
            return False
        self._ensure_preview_state(entry)
        if self._kbuf == "g" and k == "g":
            self._kbuf = ""
            self._preview_cursor_line = 0
            self._preview_cursor_col = _clamp_col(self._preview_lines, self._preview_cursor_line, self._preview_cursor_col)
            self._redraw()
            return True
        if self._kbuf == "g" and k == "e":
            self._kbuf = ""
            self._preview_cursor_col = move_word_end_backward(self._preview_lines[self._preview_cursor_line], self._preview_cursor_col)
            self._redraw()
            return True
        if k == "g":
            self._kbuf = "g"
        ...
        if k == "escape":
            ...
            return True
        if k == "/":
            self._preview_find_mode = True
            self._draw_bars()
            return True
        if k == "v":
            self._preview_visual_mode = "char"
            self._preview_anchor_line = self._preview_cursor_line
            self._preview_anchor_col = self._preview_cursor_col
            self._redraw()
            return True
        if k == "V":
            self._preview_visual_mode = "line"
            self._preview_anchor_line = self._preview_cursor_line
            self._preview_anchor_col = self._preview_cursor_col
            self._redraw()
            return True
        if k == "o" and self._preview_visual_mode:
            self._preview_anchor_line, self._preview_cursor_line = self._preview_cursor_line, self._preview_anchor_line
            self._preview_anchor_col, self._preview_cursor_col = self._preview_cursor_col, self._preview_anchor_col
            self._redraw()
            return True
        if k == "n":
            self._jump_preview_find(1)
            return True
        if k == "N":
            self._jump_preview_find(-1)
            return True
        if k in ("j", "down"):
            self._preview_cursor_line = max(0, min(len(self._preview_lines) - 1, self._preview_cursor_line + 1))
        elif k in ("k", "up"):
            self._preview_cursor_line = max(0, self._preview_cursor_line - 1)
        elif k in ("h", "left"):
            self._preview_cursor_col = max(0, self._preview_cursor_col - 1)
        elif k in ("l", "right"):
            line = self._preview_lines[self._preview_cursor_line]
            self._preview_cursor_col = min(max(0, len(line) - 1), self._preview_cursor_col + 1)
        elif k == "0":
            self._preview_cursor_col = 0
        elif k == "$":
            self._preview_cursor_col = max(0, len(self._preview_lines[self._preview_cursor_line]) - 1)
        elif k == "w":
            self._preview_cursor_col = move_word_forward(self._preview_lines[self._preview_cursor_line], self._preview_cursor_col)
        elif k == "b":
            self._preview_cursor_col = move_word_backward(self._preview_lines[self._preview_cursor_line], self._preview_cursor_col)
        elif k == "e":
            self._preview_cursor_col = move_word_end_forward(self._preview_lines[self._preview_cursor_line], self._preview_cursor_col)
        elif k == "ctrl+f" or k == "pagedown":
            self._preview_cursor_line = max(0, min(len(self._preview_lines) - 1, self._preview_cursor_line + 16))
        elif k == "ctrl+b" or k == "pageup":
            self._preview_cursor_line = max(0, self._preview_cursor_line - 16)
        elif k == "G":
            self._preview_cursor_line = max(0, len(self._preview_lines) - 1)
        elif k == "Y":
    ...
    def _yank_preview_selection(self, with_meta: bool = False) -> str:
        entry = self._cur()
        if not entry:
        ...
        if with_meta:
            line_no = self._preview_cursor_line + 1
            if self._preview_visual_mode:
                line_no = min(self._preview_anchor_line, self._preview_cursor_line) + 1
            text = f"{_display_path(entry)}:{line_no}:{self._preview_cursor_col + 1}\n{text}" if text else f"{_display_path(entry)}:{line_no}:{self._preview_cursor_col + 1}"
        return self._store_yank(text)

    def _store_yank(self, text: str) -> str:
        reg = self._active_register or '"'
        self._registers['"'] = text
        self._registers['0'] = text
        self._registers[reg] = text
        self._save_registers()
        self._active_register = '"'
        if reg == '+':
            return yank_to_clipboard(text)
        if reg == '*':
            return yank_to_primary(text)
        return f'yanked to "{reg}'

    def _registers_path(self) -> Path:
        return Path.home() / ".local" / "share" / "recall" / "registers.json"

    def _load_registers(self) -> dict[str, str]:
        path = self._registers_path()
        try:
            if path.exists():
    ...
    def _save_registers(self) -> None:
        path = self._registers_path()
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(self._registers, indent=2, sort_keys=True))

    def _get_register_text(self, reg: Optional[str] = None) -> str:
        key = reg or self._active_register or '"'
        return self._registers.get(key, "")

    def _handle_preview_find_input(self, k: str, c: str) -> None:
        if k == "escape":
            self._preview_find_mode = False
            self._draw_bars()
            return
        if k == "enter":
            self._preview_find_mode = False
            self._refresh_preview_find_hits()
            self._jump_preview_find(1, restart=True)
            return
        if k == "backspace":
            self._preview_find_query = self._preview_find_query[:-1]
            self._refresh_preview_find_hits()
            self._draw_bars()
            return
        if c and len(c) == 1:
            self._preview_find_query += c
            self._refresh_preview_find_hits()
            self._draw_bars()

    def _refresh_preview_find_hits(self) -> None:
        self._preview_find_hits = preview_search_hits(self._preview_lines, self._preview_find_query)
        self._preview_find_index = 0 if self._preview_find_hits else -1

    def _jump_preview_find(self, step: int, restart: bool = False) -> None:
        entry = self._cur()
        if not entry:
            return
        self._ensure_preview_state(entry)
        ...
        if not self._preview_find_hits:
            ...
            return
        if not restart:
            self._preview_find_index = (self._preview_find_index + step) % len(self._preview_find_hits)
        line_idx, start, _ = self._preview_find_hits[self._preview_find_index]
        self._preview_cursor_line = line_idx
        self._preview_cursor_col = start
        self._preview_cursor_col = _clamp_col(self._preview_lines, self._preview_cursor_line, self._preview_cursor_col)
        self._redraw()
